...
都市別のホテル・航空券リンクを優先したい場合は、`data/destinationAffiliateLinks.ts` に旅行先IDごとのURLを追加します。現在は診断対象30都市すべてに、A8で生成したエアトリのホテル一覧リンクを登録しています。

## おすすめホテルの編集方法

`data/recommendedHotels.ts` を編集します。旅行先IDごとにホテルを追加できます。現在はアジアの主要都市を中心に、おすすめホテル3選を登録しています。ホテル名リンクにはA8で生成したエアトリの商品リンクURLを設定しています。
